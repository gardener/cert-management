...
**Tests are a strict requirement.** Furthermore, please follow the `Boy Scouts Rule`: If you touch code for which either no tests exist or coverage is insufficient, then it is expected that **you will add relevant tests**.

This document is heavily based on the [Testing Strategy and Developer Guideline](https://gardener.cloud/docs/gardener/testing/) document of the Gardener repository. You can refer to that document for more information regarding best practices and debugging tests.
...
On macOS, press `cmd + shift + p` (or on Windows, `ctrl + shift + p`) and select 
`Go: Toggle Test Coverage In Current Package`. You have to do this every time the code changes. This will only show the coverage inside the files of the current package.
...
### Mocking
You can use [gomock](https://github.com/golang/mock) for easy mocking of interfaces. The convention is to create a `mock` directory inside the package where the mocked interface is needed.
...
To create the mocks, you should create a `doc.go` file inside the mock directory. This could look like this:
```go
...
This uses `go:generate` to create the `mocks.go` file. This is important, because that way the mocks automatically get (re)generated by running  `make generate`.
...
Here you can add the environment variables inside the `go.testEnvVars` object. With the example from above, the `settings.json` would look like this:
...
Instead of E2E tests, we use functional tests. Similar to E2E tests, functional tests **run in a Kind Cluster under "real world" conditions**, e.g., they do communicate with the **Staging Service of Let's Encrypt**. As opposed to E2E tests, functional tests test **single features and not the End-To-End usage**. In the Cert-Management project, functional tests replace the classical E2E tests.
